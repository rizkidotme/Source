...
VERSION = pkg_resources.require("bludsearch")[0].version
...
def get_text_content(md5, filename, bludsearch_dir, force, silent, encoding):
    if filename.endswith(".pdf"):
        return get_pdf_content(md5, filename, bludsearch_dir, force, silent)
...
class Document:
    def __init__(
        ...
        self.filename = filename
        self.md5 = md5
        self.bludsearch_dir = bludsearch_dir
    ...
    def content(self):
        return get_text_content(
            self.md5, self.filename, self.bludsearch_dir, False, True, self.encoding
        )
...
def process(
    ...
    # Check if bludsearch dir exists
    if not os.path.exists(bludsearch_dir):
        os.makedirs(bludsearch_dir)
    ...
    # File names
    tokens_filename = os.path.join(bludsearch_dir, get_tokens_filename(md5, config_hash))
    config_filename = os.path.join(bludsearch_dir, get_config_filename(md5, config_hash))

    should_calculate_tokens = True
    if force or not os.path.exists(tokens_filename):
        # Calculate tokens to get text chunks
        content = get_text_content(md5, filename, bludsearch_dir, force, silent, encoding)
...
@click.option(
    ...
    is_flag=True,
    default=False,
    help="Print the directory bludsearch will use to store processed files and exit",
)
@click.option(
    "--bludsearch-dir",
    type=click.Path(exists=False),
    default=None,
    help="Directory to store bludsearch files in",
)
def main(
    filename,
    windows="128_0_16",
    ...
    version=False,
    list_models=False,
    show_bludsearch_dir=False,
    bludsearch_dir=None,  # auto
):
    if version:
        print(VERSION)
        return
    ...
    if bludsearch_dir is None:
        bludsearch_dir = click.get_app_dir("bludsearch")

    if show_bludsearch_dir:
        print(bludsearch_dir)
        return
    
    # Load environment from bludsearch dir
    env_path = os.path.join(bludsearch_dir, ".env")
    ...
    def base():
        return send_from_directory(
            pkg_resources.resource_filename("bludsearch.bludsearch", "client_public"),
            "index.html",
        )
