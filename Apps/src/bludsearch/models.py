...
bludescale_model_name = "rizki/blud"
...
class OpenAIModel(BaseModel):
    def __init__(
        ...
        if "OPENAI_API_KEY" not in os.environ:
            raise Exception(
                "OpenAI API key not set. Please set the OPENAI_API_KEY environment variable or create a `.env` file with the key in the current working directory or the bludsearch directory, which is revealed by running `bludsearch --show-bludsearch-dir`."
            )
...
models = {
    ...
    "bludescale": {
        "cost_per_token": None,
        "pool_size": 15000,
        "get_model": lambda: TransformerModel(model_name=bludescale_model_name),
    },
    "sgpt": {
        "cost_per_token": None,
        "pool_size": 10000,
